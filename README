...
This README is more of a walkthrough of how the binary buddy algorithm was designed, as to understand its core principles!
...
#### Observations
1. This splitting and coalescing can be done recursively. I am going to cheat and paste an screenshot from OSTEP here.
...
Now, when the user frees the 8KB block, we(the allocator) will check whether the buddy is free. If so, we coalesce the two blocks into a 16KB block. We then check whether its buddy is free. We keep doing this up the tree until a buddy is found to be in use or we have restore the entire memory space.

2. How should we know if a block has a buddy or not? If we start with multiple freelists with initialized blocks, the initial blocks don't have buddies because they don't have a parent they split from. One option is to just start with free memory conceptually of size 2<sup>N</sup> i.e one block of size 2<sup>N</sup>. This block size is the only block that will not have a buddy. This means that initially all the other freelists for smaller fixed size blocks will be empty. Now, when splitting happens, we will be guaranteed that smaller block sizes will have a buddy.

## Overview
Now that we have walked through the design principles, let us lay a framework for the algorithm.

* Free memory is initially thought of one big space of some size 2<sup>$N$</sup>. Having memory as a power of 2 will make splitting possible since we can exactly split a block into 2 equal sized blocks.
...
* All the free lists are are implemented as doubly linked lists.
* Allocation: 
    ...
    2. For the sake of this example, lets say that $i$ is the index of the freelist in the freelists array. So, if <i>freelist[$i$]</i> contains a block, we return the block after encoding the necessary header information.
    3. Otherwise, we look for a block in a larger list <i>freelist[$j$]</i> such that $j$>$i$.
        * Recursively split that block into two until we get down to size of 2<sup>$i$</sup>.
